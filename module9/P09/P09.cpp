//P09 Array Processing - Thane Reynolds
/*
   ...
   The following items for each employee are saved in the file p09.txt:
...
void sumAndComputeAvgHours()
{
    ...
    {
        //inner for loop walks through hours (HOURS_SIZE)
        for(int i2 = 0; i2 < HOURS_SIZE; i2++)
        {
            //accumulate each individual hour into sumHours array
            sumHours[i] += hours[i][i2];
        }
        //calculate emp's avgHours using their sumHours and HOURS_SIZE.  You will be dividing, sumHours by HOURS_SIZE.
         //sumHours and HOURS_SIZE are both integers, integer division will discard digits after the decimal point
         //Need to cast one of the operands as decimal so that the values are not discarded.
         //Review division and type casting in the textbook for more information.
         //Syntax:  static_cast(HOURS_SIZE)
        avgHours[i] = (sumHours[i] / static_cast<double>(HOURS_SIZE));
    }
